...
## Planning Meetings (SR(N-1).md, sprintN.md) (max 2 marks)
  ...
  - Sprint Planning meeting (sprintN.md) (max 2 marks)
    ...
    Plz look at the feedback in the sprint 1 rubric and try to work on that moving forward.
...
## Tracking on Jira  (max 24 marks) 
  ...
  - Did you start all stories for the sprint? (max 2 marks)
    ...
    Did not provide any explanantion for why CYC-30 and CYC-33 were removed from the sprint. Need to provide explanation in the comments.
  ...
  - Is the git commit message clear? (max 2 marks)
    ...
    Your Mark: 1
    Some commit messages missing ticket number such as "set repeat to be boolean" , "Change button to Logout"
  ...
  - Tasks Distribution (max 3 marks)
    ...
    Your Mark: 3
    I would like to see bit more even task distribution. I noticed that Yi Hai Xiao (Ricky) and Tianpai Zhang worked on only one subtask compared to others who contributed to atleast more than 2 tasks  
...
## System Design (PDF or MD file or another suitable format) (max 10 marks) 
  ...
  - CRC Cards [or equivalent, if the team is not using CRC) (max 4 marks)
    ...
    Your Mark: 4
    Check out the layout given in the lectures. Usually the rectangular box (Card) layout works well to represent a CRC card.
  ...
  - Software Architecture Diagram (max 4 marks)
    ...
    Your Mark: 3
    No reference or link provided for the Three-tier architecture.
...
## Sprint Demo (Max 14 marks) 
  ...
  - Daily log
  - Repeat Task
  - Adding task to monthly log 
  - Adding task to the Future log  

  brighter colors on the forms
  layout for Register (Alert to the user), Password security missing 
---
