...
## Planning Meetings (SR(N-1).md, sprintN.md) (max 4 marks)
  ...
  - Sprint Planning meeting (sprintN.md) (max 2 marks)
    ...
    Seems like you didn't work on the sprint1 feedback. Just mentioning the hours in the team capacity do not suffice. You need to elaborate more on that.
...
## Daily Stand-ups (max 10 marks)
  ...
  Few members posted sprint 3 standup 2 and 3 few days later. Standups are supposed to be posted on the same day.
...
## User Stories (Tracked in Jira) (max 10 marks) 
  - Correctly formulated user stories (max 2 marks)
    ...
    - CYC-33,CYC-129 not well written, especially the last part of both user stories
...
## Tracking on Jira  (max 24 marks) 
  ...
  - Is each pull request made from a separate branch for each individual task? (max 5 marks)
    ...
    Feedback: There should usually be a pull request associated to each task with in a user story. I didn't penalize here because it might not have been clear. 

    For example, 3 students contributed in CYC-30, CYC-128 but only one pull request was made. Need to make pull request for each subtask.
  ...
  - Is the git commit message clear? (max 2 marks)
    ...
    Some git messages do not include ticket number. e.g "hide days and weekdays"
...
## System Design (PDF or MD file or another suitable format) (max 10 marks) 
  ...
  Also for the final presentation, make sure you use good graphical representation for your architecture diagram.  
...
## Sprint Demo (Max 14 marks) 
  ...
  Feedback: Hover over the options should show the clickable button. Mentioning the year in the future Tasks would be useful.  
